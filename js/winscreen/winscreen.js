...
import {showScreen, getSeconds, getMinutes} from '../util';
...
class WinScreen {
  ...
  init(model) {
    ...
    this.data = model.stats.win;
    this.view = new WinScreenView(this.generateResult(this.countResult()));
  ...
  generateResult(selfResults, othersResults) {
    ...
    this.data.statistics = `За ${getMinutes(timePast)} минут${ENDINGS.minutes} и ${getSeconds(timePast)} секунд${ENDINGS.seconds} вы набрали<br/>
    ${score} балл${ENDINGS.score} (${fast} быстр${ENDINGS.fast})<br/>
    совершив ${mistakes} ошиб${ENDINGS.mistakes}`;
    this.data.comparison = `Вы заняли ${place}-ое место из ${players} игрок${ENDINGS.players}. Это лучше чем у ${percent}% игроков`;
    //gameState = `win`;
